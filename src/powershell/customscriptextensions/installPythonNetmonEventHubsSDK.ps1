...
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Forcing TLS 1.2" | Out-File -FilePath $logFile -Append
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Disabling Progress" | Out-File -FilePath $logFile -Append
...
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Downloading Python for Windows 3.8.2" | Out-File -FilePath $logFile -Append
[IO.File]::WriteAllBytes($installer, (Invoke-WebRequest -Uri $uri).Content)
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Installing Python for Windows 3.8.2" | Out-File -FilePath $logFile -Append
...
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Resetting environment variables." | Out-File -FilePath $logFile -Append
...
$filename = [Uri]::new($uri).Segments | Select-Object -Last 1 | % {$_}
$installer = Join-Path -Path $($env:TEMP) -ChildPath $filename
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Downloading Network Monitor." | Out-File -FilePath $logFile -Append
[IO.File]::WriteAllBytes($installer, (Invoke-WebRequest -Uri $uri).Content)
Write-Host "[$([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fff"))] Installing Network Monitor." | Out-File -FilePath $logFile -Append
&$installer "/Q"
